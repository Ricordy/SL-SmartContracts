...
- **CFO** - The CFO is the role that manages all funds in the platform. It is responsible for withdrawing the entry fees, also withdraws the funds from all investment contracts as well as the subsequent refill of the contracts. This means that the CFO has access to all user funds when the Investment.sol contract is in PROCESS state.
